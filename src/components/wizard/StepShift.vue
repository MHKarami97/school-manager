  ...
  <div class="space-y-6">
    <div class="grid gap-3 sm:grid-cols-2">
      <button type="button" class="rounded-xl border-2 p-4 text-right transition" :class="wizard.shiftId === 'morning' ? 'border-brand-500 bg-brand-50 dark:bg-brand-500/10' : 'border-ink-200 bg-white dark:border-ink-700 dark:bg-ink-900'" @click="chooseShift('morning')">
        <p class="text-sm font-semibold text-ink-800 dark:text-ink-100">شیفت صبح</p>
      ...
      <button type="button" class="rounded-xl border-2 p-4 text-right transition" :class="wizard.shiftId === 'noon' ? 'border-brand-500 bg-brand-50 dark:bg-brand-500/10' : 'border-ink-200 bg-white dark:border-ink-700 dark:bg-ink-900'" @click="chooseShift('noon')">
        <p class="text-sm font-semibold text-ink-800 dark:text-ink-100">شیفت ظهر</p>
    ...
    <div class="grid gap-4 rounded-2xl border border-ink-100 bg-white p-5 sm:grid-cols-2 dark:border-ink-800 dark:bg-ink-900">
      <div>
        ...
        <input type="time" :value="activeConfig.startTime" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateTextField('startTime', $event)" />
      </div>
      <div>
        <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-ink-300">ساعت پایان (تقریبی)</label>
        <input type="time" :value="activeConfig.endTime" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateTextField('endTime', $event)" />
      </div>
      <div>
        <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-ink-300">طول هر زنگ (دقیقه)</label>
        <input type="number" min="20" max="90" :value="activeConfig.lessonDurationMinutes" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateNumberField('lessonDurationMinutes', $event)" />
      </div>
      <div>
        <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-ink-300">طول زنگ تفریح (دقیقه)</label>
        <input type="number" min="5" max="30" :value="activeConfig.breakDurationMinutes" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateNumberField('breakDurationMinutes', $event)" />
      </div>
      <div>
        <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-ink-300">تعداد زنگ‌های درسی در روز</label>
        <input type="number" min="4" max="10" :value="activeConfig.periodsCount" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateNumberField('periodsCount', $event)" />
      </div>
      ...
      <template v-if="activeConfig.hasLunchBreak">
        <div>
          ...
          <input type="number" min="20" max="90" :value="activeConfig.lunchDurationMinutes" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateNumberField('lunchDurationMinutes', $event)" />
        </div>
        <div>
          <label class="mb-1 block text-xs font-medium text-ink-600 dark:text-ink-300">زنگ ناهار بعد از کدام زنگ باشد؟</label>
          <input type="number" min="1" :max="activeConfig.periodsCount" :value="activeConfig.lunchAfterPeriod" class="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm text-ink-800 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100" @change="updateNumberField('lunchAfterPeriod', $event)" />
        </div>
      </template>
    </div>

    <div class="rounded-2xl border border-ink-100 bg-white p-5 dark:border-ink-800 dark:bg-ink-900">
      <p class="mb-3 text-sm font-semibold text-ink-800 dark:text-ink-100">پیش‌نمایش زنگ‌ها</p>
