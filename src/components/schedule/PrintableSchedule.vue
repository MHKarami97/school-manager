...
function teacherName(teacherId: string | null | undefined): string {
  if (!teacherId) return ''
  return props.schedule.teachers.find((t) => t.id === teacherId)?.name ?? ''
}
...
<template>
  <div class="p-6 text-ink-900" dir="rtl">
    ...
    <table class="w-full border-collapse text-xs">
      ...
      <tbody>
        <tr v-for="p in lessonPeriods" :key="p.index">
          <td class="border border-ink-400 p-2 text-center font-medium">
            زنگ {{ p.index }}<br />
            <span class="text-[10px] text-ink-500">{{ p.start }}–{{ p.end }}</span>
          </td>
    ...
    <div v-if="schedule.teachers.length" class="mt-4 flex flex-wrap gap-x-4 gap-y-1 border-t border-ink-300 pt-3 text-[10px] text-ink-600">
      <span v-for="t in schedule.teachers" :key="t.id">معلم: {{ t.name }}</span>
    </div>
  </div>
</template>
